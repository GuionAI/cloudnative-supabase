...
- Review-fix commits: `5492aa6` (`fix(platform): harden modern Supabase reconciliation`),
  ...
  `84f190f` (`fix(cnpg): track retained field ownership; superseded by the
  CNPG SSOT refactor`),
...
on both set and clear, the generated PostgreSQL projection is assigned exactly
while foreign/defaulted fields outside it survive, and durable adoption/cleanup
requires the exact project instance label. A first reconcile
...
The earlier retained-Cluster bookkeeping implementation from `84f190f` is
superseded by the CNPG SSOT refactor. The generated PostgreSQL parameters,
platform HBA rules, preload libraries, and managed roles now converge exactly
from the current SupabaseProject; no ownership annotation, parser, migration,
or compatibility path is part of the supported contract.
