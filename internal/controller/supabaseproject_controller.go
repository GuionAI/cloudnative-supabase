...
func (r *SupabaseProjectReconciler) reconcileAutoGeneratedSecrets(ctx context.Context, project *supabasev1alpha1.SupabaseProject) error {
	...
	if err == nil {
		...
		if allExist {
			...
			if project.Spec.Sequin != nil {
				if err := r.reconcileSequinSecrets(ctx, project, &secretNames); err != nil {
					return err
				}
			}
			if project.Spec.Powersync != nil {
			...
			if project.Spec.Meilisearch != nil {
				if err := r.reconcileMeilisearchSecrets(ctx, project, &secretNames); err != nil {
					return err
				}
			}
	...
	// Generate Sequin secrets if Sequin is enabled
	if project.Spec.Sequin != nil {
		if err := r.reconcileSequinSecrets(ctx, project, &secretNames); err != nil {
			return err
		}
	...
	// Generate Meilisearch secrets if Meilisearch is enabled
	if project.Spec.Meilisearch != nil {
		if err := r.reconcileMeilisearchSecrets(ctx, project, &secretNames); err != nil {
			return err
		}
...
// reconcileSequinSecrets generates Sequin-related secrets if they don't exist
func (r *SupabaseProjectReconciler) reconcileSequinSecrets(ctx context.Context, project *supabasev1alpha1.SupabaseProject, secretNames *supabasev1alpha1.SecretNamesStatus) error {
	log := logf.FromContext(ctx)

	sequinName, sequinPwdName, sequinReplPwdName := secrets.SequinSecretNames(project)

	// Check if all Sequin secrets exist
	allExist := true
	for _, name := range []string{sequinName, sequinPwdName, sequinReplPwdName} {
		existing := &corev1.Secret{}
		if err := r.Get(ctx, types.NamespacedName{Name: name, Namespace: project.Namespace}, existing); err != nil {
			if apierrors.IsNotFound(err) {
				allExist = false
	...
	if allExist {
		log.Info("Sequin secrets already exist, syncing status")
		secretNames.Sequin = sequinName
		secretNames.SequinPassword = sequinPwdName
		secretNames.SequinReplicationPassword = sequinReplPwdName
		return nil
	}

	// Generate Sequin secrets
	log.Info("Generating Sequin secrets")
	sequinSecrets, err := secrets.GenerateSequinSecrets(project)
	if err != nil {
		r.setCondition(project, supabasev1alpha1.ConditionTypeSecretsReady, metav1.ConditionFalse, "SequinSecretsFailed", err.Error())
		if statusErr := r.Status().Update(ctx, project); statusErr != nil {
			return statusErr
		}
		return err
	...
	for _, secret := range sequinSecrets {
		if err := r.createOrUpdateSecret(ctx, project, secret); err != nil {
			r.setCondition(project, supabasev1alpha1.ConditionTypeSecretsReady, metav1.ConditionFalse, "CreateFailed", err.Error())
			if statusErr := r.Status().Update(ctx, project); statusErr != nil {
				return statusErr
...
// reconcileMeilisearchSecrets generates Meilisearch-related secrets if they don't exist
func (r *SupabaseProjectReconciler) reconcileMeilisearchSecrets(ctx context.Context, project *supabasev1alpha1.SupabaseProject, secretNames *supabasev1alpha1.SecretNamesStatus) error {
	log := logf.FromContext(ctx)

	msSecretName := secrets.MeilisearchSecretName(project)

	// Check if secret exists
	existing := &corev1.Secret{}
	if err := r.Get(ctx, types.NamespacedName{Name: msSecretName, Namespace: project.Namespace}, existing); err == nil {
		log.Info("Meilisearch secrets already exist, syncing status")
		secretNames.MeilisearchMasterKey = msSecretName
		return nil
	} else if !apierrors.IsNotFound(err) {
		return err
	}

	// If user provided a secret ref, just use it (no generation needed)
	...
	// Generate Meilisearch secrets
	log.Info("Generating Meilisearch secrets")
	msSecrets, err := secrets.GenerateMeilisearchSecrets(project)
	if err != nil {
		r.setCondition(project, supabasev1alpha1.ConditionTypeSecretsReady, metav1.ConditionFalse, "MeilisearchSecretsFailed", err.Error())
		if statusErr := r.Status().Update(ctx, project); statusErr != nil {
			return statusErr
		}
		return err
...
func (r *SupabaseProjectReconciler) reconcileSequin(ctx context.Context, project *supabasev1alpha1.SupabaseProject) error {
	...
	if err := r.createOrUpdateDeployment(ctx, project, deployment); err != nil {
		r.setCondition(project, supabasev1alpha1.ConditionTypeSequinReady, metav1.ConditionFalse, "DeploymentFailed", err.Error())
		if statusErr := r.Status().Update(ctx, project); statusErr != nil {
			return statusErr
		}
		return err
	...
	// Create service
	service := services.BuildSequinService(project)
	if err := r.createOrUpdateService(ctx, project, service); err != nil {
		r.setCondition(project, supabasev1alpha1.ConditionTypeSequinReady, metav1.ConditionFalse, "ServiceFailed", err.Error())
		if statusErr := r.Status().Update(ctx, project); statusErr != nil {
			return statusErr
		}
		return err
...
func (r *SupabaseProjectReconciler) reconcilePowersync(ctx context.Context, project *supabasev1alpha1.SupabaseProject) error {
	log := logf.FromContext(ctx)
	...
	psConfig := configmaps.BuildPowersyncConfigMap(project, dbHost)
	...
	syncRules := configmaps.BuildPowersyncSyncRulesConfigMap(project)
	if syncRules != nil {
		if err := r.createOrUpdateConfigMap(ctx, project, syncRules); err != nil {
	...
	project.Status.Services.PowersyncAPI = supabasev1alpha1.ServiceStatus{Ready: true}
	project.Status.Services.PowersyncReplication = supabasev1alpha1.ServiceStatus{Ready: true}
...
// reconcileMeilisearch deploys the Meilisearch service (StatefulSet + Service)
func (r *SupabaseProjectReconciler) reconcileMeilisearch(ctx context.Context, project *supabasev1alpha1.SupabaseProject) error {
	log := logf.FromContext(ctx)
	log.Info("Reconciling Meilisearch service")

	secretNames := &project.Status.SecretNames

	// Create StatefulSet
	sts := deployments.BuildMeilisearchStatefulSet(project, secretNames)
	if err := r.createOrUpdateStatefulSet(ctx, project, sts); err != nil {
		r.setCondition(project, supabasev1alpha1.ConditionTypeMeilisearchReady, metav1.ConditionFalse, "StatefulSetFailed", err.Error())
		if statusErr := r.Status().Update(ctx, project); statusErr != nil {
			return statusErr
		}
		return err
	...
	// Create service
	service := services.BuildMeilisearchService(project)
	if err := r.createOrUpdateService(ctx, project, service); err != nil {
		r.setCondition(project, supabasev1alpha1.ConditionTypeMeilisearchReady, metav1.ConditionFalse, "ServiceFailed", err.Error())
		if statusErr := r.Status().Update(ctx, project); statusErr != nil {
			return statusErr
		}
		return err
	...
	project.Status.Services.Meilisearch = supabasev1alpha1.ServiceStatus{Ready: true}
	r.setCondition(project, supabasev1alpha1.ConditionTypeMeilisearchReady, metav1.ConditionTrue, "Ready", "Meilisearch service is running")
	return nil
}

// createOrUpdateStatefulSet creates or updates a StatefulSet resource
func (r *SupabaseProjectReconciler) createOrUpdateStatefulSet(ctx context.Context, project *supabasev1alpha1.SupabaseProject, sts *appsv1.StatefulSet) error {
	log := logf.FromContext(ctx)
	...
	// Update existing - only update mutable fields (VolumeClaimTemplates are immutable)
